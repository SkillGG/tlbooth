...
export class Mutation {
    ...
    constructor(id: string, fn: ((p: StoreNovel[]) => StoreNovel[]), desc: MutationDescription, type: MutationType, apiFn: () => Promise<void>, dependencies: Dependency[]) {
        this.fn = fn
        this.id = id;
        this.desc = desc;
        this.type = type
...
export const useNovelStore = create<NovelStore>()((set, get) => ({
    ...
    undo: (id) => {
        set(s => {
            ...
            if (infT)
                return { mutations: s.mutations.filter(t => t.id !== id), undoneMutations: [...s.undoneMutations, infT] }
            else return s
        })
    },
    redo: (id) => {
        set(s => {
            ...
            if (!infT) return s;
            return { mutations: [...s.mutations, infT], undoneMutations: s.undoneMutations.filter(t => t.id !== id) }
        });
    },
    getMutated: () => {
